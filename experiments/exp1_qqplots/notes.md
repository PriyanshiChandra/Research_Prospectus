...
where $F_n$ is the empirical CDF of the $B$ standardised statistics and $\Phi$ is the standard normal CDF. Under $H\_0: Z\_n \sim \mathcal{N}(0,1)$, $D\_n \sqrt{B} \xrightarrow{d}$ Kolmogorov distribution. A large $p$-value means we fail to reject normality.
